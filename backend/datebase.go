...
const GET_VIDEOS= "SELECT v.id, TRIM(v.title), v.publishedat, TRIM(ch.title) as ptitle, v.duration FROM video v" +
	" LEFT JOIN channel ch ON ch.id = v.idch" +
	" WHERE ch.enable = true ORDER BY publishedat DESC LIMIT $1 OFFSET $2"
	
const GET_VIDEOS_BY_ID_CHANNEL = "SELECT id, TRIM(title), publishedat, '' ptitle, v.duration FROM video v" + 
	" WHERE idch = $1 " +
...
func getVideoByIdFromDB(id string) ( *YoutubeVideo, error) {
	...
	err := db.QueryRow(GET_VIDEO_BY_ID, id).Scan(&title, &description, &idch, &chtitle, &publishedat, &count_metrics, &min_timemetric, &max_timemetric )
	if err != nil {
	...
	youtubeVideo := &YoutubeVideo{strings.TrimSpace(title), strings.TrimSpace(description), strings.TrimSpace(idch), 
			strings.TrimSpace(chtitle), publishedat, count_metrics, max_timemetric, min_timemetric}
	
	Logger.Debugf("id: %v, title: %v, description: %v, idch: %v, chtitle: %v, publishedat: %v, count_metrics: %v, max_timemetric: %v, min_timemetric: %v", 
			id, title, description, idch, chtitle, publishedat, count_metrics, max_timemetric, min_timemetric)
...
func getVideosByChannelIdFromDB(id string, offset int) ([]byte, error) {
	...
	for rows.Next() {
		...
		rows.Scan(&id, &title, &publishedat, &ptitle, &duration)

		response = append(response, YoutubeVideoShort{id, title, publishedat, ptitle, duration})
	}
	err = rows.Err()
	if err != nil {
		Logger.Error(err)
